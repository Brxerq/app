...
/**
 * A scribbled underline that gets drawn under its children on hover and
 * un-drawn on leave — a different scribble each time, so no two hovers match.
 *
 * Wraps inline content; the SVG sits under it and never affects layout.
 */
...
// Shared across every instance so consecutive hovers cycle instead of repeating.
let nextIndex: number | null = null;

function pickVariant(): PathData {
  nextIndex =
    nextIndex === null
...
export function DrawnUnderline({
  ...
  const leaveRef = useRef<gsap.core.Tween | null>(null);
  const [variant, setVariant] = useState<PathData | null>(null);

  // Draw the scribble in whenever a new variant is mounted.
  useEffect(() => {
    const path = pathRef.current;
    if (!path || !variant) return;

    leaveRef.current?.kill();
  ...
  const handleLeave = () => {
    ...
    const drawOut = () => {
      if (leaveRef.current?.isActive()) return;
      if (prefersReducedMotion()) {
        setVariant(null);
        return;
      }
      leaveRef.current = gsap.to(path, {
        drawSVG: '100% 100%',
        duration: 0.5,
        ease: 'power2.inOut',
        onComplete: () => {
    ...
    // Let an in-flight draw finish before erasing it.
    if (enterRef.current?.isActive()) {
      enterRef.current.eventCallback('onComplete', drawOut);
    } else {
      drawOut();
    }
  };
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden
        className="pointer-events-none absolute inset-x-0 -bottom-1.5 block h-2 overflow-visible"
      >
        {variant && (
          <svg
